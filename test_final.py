...
known_rounds = [
    ('试选', lambda s: '试选' in s),
    ('海选', lambda s: '海选' in s),
    ('第一轮抢选', lambda s: bool(re.search(r'^\d*[、．]\s*第一轮抢选', s))),
    ('第二轮抢选', lambda s: bool(re.search(r'^\d*[、．]\s*第二轮抢选', s))),
...
for label, url in URLS.items():
    ...
    # Post-process: split 抢选
    def day_year(m, d):
        days = d
        for i in range(1, m): days += (28 if i == 2 else 30 if i in (4,6,9,11) else 31)
        return days
    ...
    for i in range(len(ext_rounds)):
        for j in range(i+1, len(ext_rounds)):
            ...
            aS = day_year(a['sm'], a['sd']); aE = day_year(a['em'], a['ed'])
            bS = day_year(b['sm'], b['sd']); bE = day_year(b['em'], b['ed'])
            aEF = aE + (365 if a['sm'] > a['em'] else 0)
            bEF = bE + (365 if b['sm'] > b['em'] else 0)
            aW = a['sm'] > a['em']; bW = b['sm'] > b['em']
            aSF = aS + (0 if aW else (365 if bW and a['sm'] < b['sm'] else 0))
            bSF = bS + (0 if bW else (365 if aW and b['sm'] < a['sm'] else 0))
            if aSF <= bSF and aEF >= bEF: main, pause = a, b
            elif bSF <= aSF and bEF >= aEF: main, pause = b, a
            else: continue
