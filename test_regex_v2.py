#!/usr/bin/env python3
"""Test updated regex patterns against actual pages"""
...
# Updated regex (same as TypeScript)
TP = re.compile(
...
for label, url in URLS.items():
    ...
    # Extract segments (same logic as TypeScript)
    # First try red blocks
    ...
    # Fallback to plain text split by numbered prefixes
    if not segments:
    ...
    for seg in segments:
        ...
        if tm:
            ...
            print(f'  {name:12s} → {ext_rounds[-1]["start"]} ~ {ext_rounds[-1]["end"]}  (周{ext_rounds[-1]["week"]})')
        else:
            print(f'  {name:12s} → (NO TIME MATCH): {seg[:60]}')
    
    # Post-process: split 抢选 by pause
    qx_idx = next((i for i, r in enumerate(ext_rounds) if r['name'] == '抢选'), -1)
    pause_idx = next((i for i, r in enumerate(ext_rounds) if r['name'] == '__pause__'), -1)
    if qx_idx != -1 and pause_idx != -1:
        print('\n  *** Split 抢选 by pause ***')
        ...
        if md(pause['start']) >= md(qx['start']) and md(pause['end']) <= md(qx['end']):
            # Phase 1: qx start -> pause start
            p1 = dict(qx)
            p1['name'] = '抢选（第一阶段）'
            p1['end'] = pause['start']
            # Phase 2: pause end -> qx end
            p2 = dict(qx)
            p2['name'] = '抢选（第二阶段）'
            p2['start'] = pause['end']
            ...
            print(f'    第一阶段: {p1["start"]} ~ {p1["end"]}')
            print(f'    第二阶段: {p2["start"]} ~ {p2["end"]}')
